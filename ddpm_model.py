...
class DiffusionModel(pl.LightningModule):
    ...
    def forward(self, x, t):
        """
        Model is U-Net with added positional encodings and self-attension layers.
        """
        x1 = self.inc(x)
    ...
    def get_loss(self, batch, batch_idx):
        ...
        noise_imgs = []
        e = torch.randn(batch.shape, device=self.device)
        for i in range(len(ts)):
            a_hat = self.alpha_bar(ts[i])
            noise_imgs.append(
                (math.sqrt(a_hat) * batch[i]) + (math.sqrt(1. - a_hat) * e[i])
            )
        
        noise_imgs = torch.stack(noise_imgs, dim=0)
        e_hat = self.forward()
        loss = F.mse_loss(
            e_hat.reshape(-1, self.in_size), e.rehape(-1, self.in_size)
        )
        return loss
